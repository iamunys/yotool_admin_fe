...
import { FollowupsEmptyState, FollowupsErrorState, FollowupsTable, FollowupsTableSkeleton } from "@/components/FollowupsTable";
import { useAdminFollowups } from "@/hooks/useAdminFollowups";
import { useDebouncedValue } from "@/hooks/useDebouncedValue";
import { FOLLOWUP_STATUSES, type AdminFollowupStatus } from "@/types/admin";

const PAGE_SIZE = 20;

type SortValue = "created_at-desc" | "created_at-asc" | "due_date-desc" | "due_date-asc";
...
const statusOptions: { value: AdminFollowupStatus | ""; label: string }[] = [
  { value: "", label: "All statuses" },
  ...FOLLOWUP_STATUSES.map((status) => ({ value: status, label: formatStatusOption(status) })),
];

function formatStatusOption(status: AdminFollowupStatus): string {
...
export default function FollowupsPage() {
  ...
  const [statusFilter, setStatusFilter] = useState<AdminFollowupStatus | "">("");
  const [sort, setSort] = useState<SortValue>("created_at-desc");
  const [page, setPage] = useState(1);

  const debouncedSearch = useDebouncedValue(searchInput, 350);
  const [sortBy, sortOrder] = sort.split("-") as ["created_at" | "due_date", "asc" | "desc"];
  ...
  const resetKey = `${debouncedSearch}|${statusFilter}|${sort}`;
  ...
  const { state, retry } = useAdminFollowups({
  ...
  return (
    <section aria-label="Follow-ups">
      ...
      <div className="mt-5">
        {state.status === "loading" && <FollowupsTableSkeleton />}

        {state.status === "error" && <FollowupsErrorState message={state.message} onRetry={retry} />}

        {state.status === "ready" && state.data.length === 0 && (
          <FollowupsEmptyState hasFilters={hasFilters} onClearFilters={clearFilters} />
        )}

        {state.status === "ready" && state.data.length > 0 && (
          <>
            <FollowupsTable followups={state.data} />
            <Pagination
              page={state.page}
...
function SearchInput({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    <div className="relative w-full sm:max-w-xs">
      <label htmlFor="followup-search" className="sr-only">Search follow-ups</label>
      <svg
        aria-hidden="true"
        viewBox="0 0 24 24"
        fill="none"
